...
function boilerstrap_setup() {
	...
	register_nav_menu( 'top', __( 'Top Menu', 'boilerstrap' ) );
	register_nav_menu( 'footer', __( 'Footer Menu', 'boilerstrap' ) );
	/*
...
function build_taxonomies() {
   // Positions
   register_taxonomy( 'thematic', 'post', array( 'hierarchical' => true, 'label' => 'Θεματική', 'query_var' => true, 'rewrite' => true ) );
}
...
function boilerstrap_widgets_init() {
	...
	register_sidebar( array(
		'name' => __( 'Third Front Page Widget Area', 'boilerstrap' ),
		'id' => 'sidebar-4',
		'description' => __( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'boilerstrap' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
...
function boilerstrap_entry_meta() {
	...
	$document = "<p class='well well-small'>";
	if( $categories_list){
		$document .="<span class='badge badge-info'>";
		$document .=  __( 'Categories', 'boilerstrap');
		$document .=":</span> ".$categories_list ."<br/>";
	}
	
	if ( $tag_list ){
	$document .= "<span class='badge badge-info'>";
	$document .=  __( 'Tags', 'boilerstrap');
	$document .=":</span> ".$tag_list ."<br/>";	
	}
	$document .="</p>";
	}
	// __( ' %1$s', 'boilerstrap' ).";
...
function get_the_post_image($width=120, $id = null, $claz = null) {
	global $post, $posts;
	...
	if(!(empty($first_img))){ 
		if(!empty($claz))
			$img = "<img src=\"$first_img\" alt=\"".$post->post_title."\" width=\"$width\" class=\"$claz\"/>";
		else
			$img = "<img src=\"$first_img\" alt=\"".$post->post_title."\" width=\"$width\"/>";
		return $img;
	}else{
		return get_video_thumbnail($width, false, $id) ;
	}
}

/* Return YouTube Embeded Video Thumbnail ************************/
function get_video_thumbnail($width=150, $alt=false, $id = null) {
	...
	preg_match_all ('/'.$regex_pattern.'/s', $post->post_content, $matches);
	//print_r($regex_matches);
	if (array_key_exists( 2, $matches )) {
		if(in_array( 'bliptv', $matches[2] ) ) {
			$key = array_search('bliptv', $matches[2]);
			$video_url = explode('-',$matches[3][$key]);
			$position = count($video_url) - 1;
			$rss_lnk = 'http://blip.tv/rss/view/'.$video_url[$position];
			$xml = simplexml_load_file($rss_lnk);
			$imgURL = $xml->channel->item->children('http://blip.tv/dtd/blip/1.0')->thumbnail_src;
			$imgURL = 'http://a.images.blip.tv/'.$imgURL;
			return '<img src="'.$imgURL.'" alt="'.$post->post_title.'" width="'.$width.'" />';
		}
		if(in_array( 'youtube', $matches[2] ) ) {
			$key = array_search('youtube', $matches[2]);
			parse_str( parse_url( $matches[3][$key], PHP_URL_QUERY ), $video_vars );
			return '<img src="http://img.youtube.com/vi/'.$video_vars['v'].'/0.jpg" alt="'.$post->post_title.'" width="'.$width.'" />';
		}
		if(in_array( 'vimeo', $matches[2] ) ) {
			$key = array_search('vimeo', $matches[2]);
			$video_url = parse_url(trim($matches[3][$key]), PHP_URL_PATH);
			$hash = unserialize(file_get_contents("http://vimeo.com/api/v2/video".$video_url.".php"));
			return '<img src="'.$hash[0]["thumbnail_large"].'" alt="'.$post->post_title.'" width="'.$width.'" />';
		}
	}
}

function youtube_shortcode($atts)  {
	parse_str( parse_url( $atts[0], PHP_URL_QUERY ), $video_vars ); 
	return  '<iframe width="690" height="440" src="http://www.youtube.com/embed/'.$video_vars['v'].'?rel=0" frameborder="0" allowfullscreen></iframe>' ;
}
add_shortcode("youtube", "youtube_shortcode");

function vimeo_shortcode($atts)  {
	$video_url = parse_url($atts[0]);
	return '<iframe src="http://player.vimeo.com/video/'.substr($video_url['path'], 1).'" width="690" height="440" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
}
add_shortcode("vimeo", "vimeo_shortcode");

function blip_shortcode($atts)  {
	$video_url = explode('-',$atts[0]);
	$position = count($video_url) - 1;
	$rss_lnk = 'http://blip.tv/rss/view/'.$video_url[$position];
	$xml = simplexml_load_file($rss_lnk);
	$embedURL = $xml->channel->item->children('http://blip.tv/dtd/blip/1.0')->embedLookup;
	return '<iframe src="http://blip.tv/play/'.$embedURL.'.html?p=1" width="690" height="440" frameborder="0" allowfullscreen></iframe><embed type="application/x-shockwave-flash" src="http://a.blip.tv/api.swf#'.$embedURL.'" style="display:none"></embed>';
}
add_shortcode("bliptv", "blip_shortcode");

include_once('first-page-options.php');
?>